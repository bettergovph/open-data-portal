 ...
 *
 * This script reads sample-data.json and generates SQL statements to seed the database.
 *
 * Usage:
 * 1. Run this script: npm run seed
 * 2. Execute the generated SQL:
 *    - Local: npx wrangler d1 execute open-data-db --local --file=./scripts/seed-generated.sql
 *    - Remote: npx wrangler d1 execute open-data-db --file=./scripts/seed-generated.sql
 */
...
for (const dataset of data.datasets) {
  ...
  const values = [
    ...
    totalSize.toString(),
    dataset.latest_version_date ? `'${escapeSql(dataset.latest_version_date)}'` : 'NULL',
  ];
...
console.log('');
console.log('  pnpx wrangler d1 execute open-data-db --local --file=./scripts/init.sql');
